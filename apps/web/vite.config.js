...
// Keep dependency optimization narrow in dev. Pre-bundling the entire dependency
// graph here can stall startup and leave Vite stuck in a temp deps directory.
const optimizeDepsInclude = allDeps.filter((d) =>
	['react', 'react-dom', 'react-router-dom', 'framer-motion', 'lucide-react'].includes(d)
);
...
const addTransformIndexHtml = {
	...
	transformIndexHtml(html) {
		if (!isDev && process.env.HORIZONS_ENABLE_RUNTIME_PATCHES !== '1') {
			return { html, tags: [] };
		}
