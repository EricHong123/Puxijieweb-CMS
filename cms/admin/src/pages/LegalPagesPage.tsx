  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-warm-charcoal">法律页面</h1>
        ...
        {LOCALES.map((l) => (
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${
              locale === l ? 'bg-[hsl(var(--card))] text-warm-charcoal shadow-sm' : 'text-warm-charcoal-muted hover:text-warm-charcoal'
            }`}
          >
            {l.toUpperCase()}
          </button>
      ...
      <div className="space-y-3">
        {PAGE_TYPES.map((pt) => {
          ...
          return (
            <div key={pt.key} className="bg-[hsl(var(--card))] rounded-xl border overflow-hidden">
              {isExpanded && editData ? (
                <div className="p-5 space-y-4">
                  <h3 className="font-semibold text-warm-charcoal">{pt.label} — 编辑 ({locale.toUpperCase()})</h3>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h4 className="text-sm font-semibold text-warm-charcoal">章节内容</h4>
                    ...
                    {editData.sections.map(([title, paragraphs], si) => (
                      <div key={si} className="border rounded-lg p-3 space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => removeSection(si)} className="p-1 text-warm-charcoal-muted/60 hover:text-red-500"><X className="h-4 w-4" /></button>
                        </div>
                        {paragraphs.map((p, pi) => (
                          <div key={pi} className="flex gap-2">
                            ...
                            <button onClick={() => removeParagraph(si, pi)} className="p-1 text-warm-charcoal-muted/60 hover:text-red-500 shrink-0"><X className="h-3.5 w-3.5" /></button>
                          </div>
                        ))}
                ...
                <button
                  ...
                  className="w-full p-5 flex items-center gap-4 hover:bg-[hsl(var(--secondary))] text-left transition-colors"
                >
                  <Scale className="h-5 w-5 text-primary shrink-0" />
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-warm-charcoal">{page?.title || pt.label}</div>
                    {page?.description && (
